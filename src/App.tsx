...
export default function App() {
  ...
  const [activeSong, setActiveSong] = useState<{ videoId: string; title: string } | null>(null);
  ...
  const playSong = useCallback(async (query: string) => {
    try {
      ...
      if (!res.ok) throw new Error("Search song failed");
      ...
      if (data && data.videoId) {
        ...
        setMessages((prev) => [
          ...prev, 
          { id: Date.now().toString() + "-z", sender: "zoya", text: `🎶 Abhi gata hu Himanshu: "${data.title || query}"! Swag se suno!` }
        ]);
      ...
      setMessages((prev) => [
        ...prev, 
        { id: Date.now().toString() + "-z", sender: "zoya", text: "Uff, technical error aa rahi hai. Internet connection check karo Himanshu!" }
      ]);
      setAppState("idle");
    }
  }, []);
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isSongPlay && commandResult.songQuery) {
      ...
      await playSong(commandResult.songQuery);
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-serif font-medium tracking-wide opacity-90">Sadhna Sinha</h1>
        </div>
      ...
      {/* Dynamic Song Player Overlay */}
      <AnimatePresence>
        {activeSong && (
          <div className="absolute right-4 bottom-24 md:right-12 md:bottom-28 z-30 pointer-events-auto max-w-[90vw]">
            <SongPlayer
          ...
          {showTextInput && (
            ...
            >
              <input 
                ...
                placeholder="Type a message to Sadhna Sinha..."
